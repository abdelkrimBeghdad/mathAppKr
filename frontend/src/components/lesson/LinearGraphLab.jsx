...
function LinearGraphContent({ phase, setPhase }) {
    ...
    const generateProblem = () => {
        ...
        let newA = (Math.floor(Math.random() * maxCoeff) + 1) * (Math.random() > 0.5 ? 1 : -1);
        if (Math.random() > 0.5) newA = newA / 2;

        setA(newA);
        setPointPos({ x: 0, y: 0 });
        setPhase('practice');
        setError(false); setFeedback(null); setIsCompleted(false); setReward(null);
        labProgressService.update('linear-graph', 'practice').catch(() => { });
    };
    ...
    const handleCheck = async () => {
        if (pointPos.x !== 0 && Math.abs(pointPos.y / pointPos.x - a) < 0.1) {
            ...
            setError(false);
            await labProgressService.update('linear-graph', 'completed', 100).catch(() => { });
            try {
                const data = await rewardService.claimLabReward('linear-graph-mastery');
        ...
        <div className="w-full max-w-3xl px-2">
            ...
            <div className="flex justify-between items-center mt-6 px-4">
                ...
                {learnStep < learnPages.length - 1
                    ...
                    : <button onClick={generateProblem} className="px-8 py-3 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl font-black">بدء الإسقاط</button>
                }
            </div>
        </div>
    );
