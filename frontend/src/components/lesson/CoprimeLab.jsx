...
function CoprimeContent({ phase, setPhase }) {
    ...
    const challengeList = [
        { a: 25, b: 27, coprime: true }, { a: 12, b: 18, coprime: false },
        { a: 14, b: 15, coprime: true }, { a: 8, b: 20, coprime: false },
    ...
    const current = challengeList[currentChallenge];

    const resetChallenges = () => {
        setCurrentChallenge(0); setChallenges([]); setFeedback(null); setReward(null);
        setPhase('practice');
    };

    const handleChallengeAnswer = async (answer) => {
        const correct = answer === current.coprime;
        const g = gcd(current.a, current.b);
        const newChallenges = [...challenges, { ...current, userAnswer: answer, correct, pgcd: g }];
        ...
        if (currentChallenge < challengeList.length - 1) {
            setTimeout(() => { setCurrentChallenge(c => c + 1); setFeedback(null); }, 1200);
        } else {
    ...
    if (phase === 'intro') return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full max-w-4xl px-2">
            <div className={`p-6 rounded-[1rem] border backdrop-blur-3xl ${theme.card}`}>
                ...
                <p className={`${theme.textSub} text-sm mb-4 font-medium`}>تعلم القاعدة الذهبية والفرق بين العدد الأولي والعددين الأوليين فيما بينهما.</p>
            ...
            <motion.button onClick={resetChallenges} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="relative rounded-[1rem] shadow-2xl overflow-hidden">
        ...
        <div className="w-full max-w-3xl px-2">
            ...
            <div className="flex justify-between items-center mt-6 px-4">
                ...
                {learnStep < 1
                    ...
                    : <button onClick={resetChallenges} className="px-8 py-3 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl font-black">تحدي الآن</button>
                }
            </div>
        </div>
    );
    ...
    return (
        <>
            <LabChallenge
                ...
                level={currentChallenge < 2 ? 1 : currentChallenge < 4 ? 2 : 3}
